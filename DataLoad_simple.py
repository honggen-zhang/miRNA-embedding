...
def split_into_six_mers(sequence):
    six_mers = [sequence[i:i+4] for i in range(0, len(sequence), 4)]
...
class RNA_Seq(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        starts_one = int((self.starts[index])/4)+1
        ends_one = int((self.ends[index])/4)+1
        target = int(self.target[index])
        #label = np.array(label, dtype=np.float32)
        rna_ = split_into_one_mers(rna_)
        self.tokenizer_1mer.model_max_length = 40
        ...
        return mirna_input, mirna_attention,target_site_input,target_site_attention, dna_input,dna_attention,target
...
class RNA_Seq_eval(data.Dataset):
    ...
    def __getitem__(self, index):
        rna_, dna_, target_site_ = self.data[index]
        starts_one = int((self.starts[index])/4)
        ends_one = int((self.ends[index])/4)+1
        target = int(self.target[index])
        #label = np.array(label, dtype=np.float32)
        rna_ = split_into_one_mers(rna_)
        ...
        return mirna_input, mirna_attention, dna_input,dna_attention,starts_one,ends_one,target,target_site_
